...
export function buildToolFilter(config: {
  ...
  // Read-only mode is implemented through this filter rather than through a
  // registration gate in server.ts, because `registerClientTools` registers the
  // read and the write client tools together and splitting it would be a much
  // larger change than the guarantee needs. The observable result is the same:
  // a write tool is removed from the SDK's tool map before the server ever
  // connects, so it is absent from `tools/list` and answers `tools/call` with
  // "tool not found". Nothing is refused at call time.
  if (allow === undefined && deny === undefined && !config.readOnly) {
    return { active: false, selected: new Set() };
  }

  // What would be registered without any filter. In read-only mode the write
  // tools are dropped, but they stay in the catalogue so that a name from that
  // half is answered with "read-only suppresses it", never with "no such tool".
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      const matches = expand(entry, 'WG_EASY_ALLOW_TOOLS');
      if (matches.length === 0) {
        throw new ToolFilterError(
          `WG_EASY_ALLOW_TOOLS: no tool matches "${entry}". ` +
      ...
      if (survivors.length === 0) {
        if (entry.endsWith('*')) {
          ...
          console.error(
            `wg-easy-mcp: WG_EASY_ALLOW_TOOLS: "${entry}" matches only write ` +
              'tools, which WG_EASY_READ_ONLY suppresses — it contributes nothing.'
          );
        ...
        throw new ToolFilterError(
          `WG_EASY_ALLOW_TOOLS: "${entry}" is a write tool, but WG_EASY_READ_ONLY ` +
            'is set — it is never registered. Remove it from WG_EASY_ALLOW_TOOLS, ' +
            `or unset WG_EASY_READ_ONLY. Available in read-only mode: ${[...READ_TOOLS].sort().join(', ')}.`
        );
      }
  ...
  for (const entry of deny ?? []) {
    ...
    const matches = expand(entry, 'WG_EASY_DENY_TOOLS');
    if (matches.length === 0) {
      throw new ToolFilterError(
        `WG_EASY_DENY_TOOLS: no tool matches "${entry}". Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'WG_EASY_ALLOW_TOOLS selects only write tools, but WG_EASY_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'WG_EASY_ALLOW_TOOLS/WG_EASY_DENY_TOOLS leave no tools registered — the ' +
